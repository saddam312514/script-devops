...
def prepare_booking_request(frm, to, doj, seat_class, trip_id, trip_route_id, boarding_point_id, name, mobile, email, no_of_tickets, tickets):
    payload = {
        "from_city": frm,
        "to_city": to,
        "date_of_journey": doj,
        ...
        "ticket_ids[]": [],
        "pname[]": [name], #* no_of_tickets,
        "passengerType[]": "Adult", #* no_of_tickets,
        "gender[]": "male", #* no_of_tickets,
        "pmobile": mobile,
        "pemail": email,
        "contactperson": 0,
        "selected_mobile_transaction": 1,
    ...
    for ticket in tickets:
        payload["ticket_ids[]"]= ticket["ticket_id"]
    
    from urllib.parse import quote, quote_plus
        

    part1 = "from_city=" + quote(frm) + "&to_city=" + quote(to) +  "&date_of_journey=" + doj + "&seat_class=" + seat_class + "&trip_id=" + trip_id + "&trip_route_id="+ trip_route_id + "&boarding_point_id=" + str(boarding_point_id) + "&" + quote("ticket_ids[]") + "=" + str(tickets[0]["ticket_id"]) 
    part2 =  "&" + quote("pname[]") + "=" + quote("name") + "&"+ quote("passengerType[]") + "=Adult&" + quote("gender[]") + "=male&pmobile=" + mobile + "&pemail=" + quote(email) + "&contactperson=0&selected_mobile_transaction=1&is_bkash_online=1"
